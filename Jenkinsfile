pipeline {
    ...
    stages {
        ...
        stage("Checkout from SCM") {
            steps {
                git branch: 'main',
                    credentialsId: 'gowdrr001',
                    url: 'https://github.com/gowdrr001/register-app.git'
            }
        }
        ...
        stage("Trigger CD Pipeline") {
            steps {
                script {
                    ...
                    --data "IMAGE_TAG=${RELEASE}" \
                    'http://192.168.60.135:8080/job/gitops-project/build?token=gitops-token'
                    """
                }
            }
        }
